<script lang="ts">
	...
	async function handleDelete() {
		isDeleting = true;
		try {
			const result = await callS3<{ success?: boolean; deleted?: number; error?: string }>(
				"deleteFolder",
				{ connectionId, bucket: bucketName, folderPrefix: folderKey },
			);
			if (result.success) {
				toast.success(
					(result.deleted ?? 0) > 0
...
>
	...
	>
		<div class="space-y-4">
			...
			<div class="rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm space-y-1.5">
				{#if itemCount === null}
				...
				{:else}
					<p class="font-medium text-destructive">
						{itemCount} item{itemCount === 1 ? "" : "s"} inside this folder
					</p>
					<p class="text-muted-foreground">
						This action is permanent. The folder and everything in it will be deleted.
					</p>
				{/if}
			</div>
